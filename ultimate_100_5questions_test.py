#!/usr/bin/env python3
"""Ultimate test: 100 concurrent + 5 questions sequentially."""
...
class UltimateLLMClient:
    """Ultimate LLM Client with proven balanced approach."""
...
Your response:"""
    ...
    async def query_all_personas_balanced(self, personas, question):
        ...
        print(f"Querying {len(personas)} personas with ULTIMATE concurrency (100)")
        ...
        async with aiohttp.ClientSession(
            connector=connector, 
            timeout=timeout,
            headers={"User-Agent": "Ultimate-LLM-Client/4.0"}
        ) as session:
...
async def test_ultimate_100_5questions():
    """Test 100 concurrent + 5 questions sequentially."""
    print("🚀 ULTIMATE 100 CONCURRENT + 5 QUESTIONS TEST")
    print("=" * 70)
    ...
    print(f"      - ultimate_responses_q1.json through ultimate_responses_q5.json")
    print(f"      - ultimate_analysis_q1.json through ultimate_analysis_q5.json")
    ...
    for q_idx, question in enumerate(questions, 1):
        ...
        filename = f'ultimate_responses_q{q_idx}.json'
        ...
        analysis = analyzer.analyze_responses(results, question)
        
        analysis_filename = f'ultimate_analysis_q{q_idx}.json'
    ...
    if avg_quality >= 95:
        print(f"   🟢 EXCELLENT: RTX 5090 handles 100 concurrent perfectly!")
    elif avg_quality >= 80:
        print(f"   🟡 GOOD: RTX 5090 handles 100 concurrent well")
    elif avg_quality >= 60:
        print(f"   🟠 FAIR: RTX 5090 manages 100 concurrent")
    else:
        print(f"   🔴 STRUGGLING: 100 concurrent may be too high")
    
    print(f"\n📁 ALL FILES CREATED:")
    for q_idx in range(1, len(questions) + 1):
        print(f"   - ultimate_responses_q{q_idx}.json")
        print(f"   - ultimate_analysis_q{q_idx}.json")


if __name__ == "__main__":
    asyncio.run(test_ultimate_100_5questions())
